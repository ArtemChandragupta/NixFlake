Sideberry does not support declarative configuration, so extra steps are needed:

1) In `Tabs` page:
    - Pinned tabs position to _globally - top_

2) In `Styles editor` page set:
    - Border radius to _7px_;
    - Toolbar background to #282828
    - Frame background to #282828

3) In the `Styles editor` page copy
```css
#root.root {--frame-bg: #282828;}
#root.root {--toolbar-bg: #282828;}

#root.root {--general-border-radius: 7px;}


/* Replacing Sidebery 'settings' with Fluent settings icon. */
...
/* Replacing 'add tabs panel' button with Fluent plus icon. */
